...
with tab_funding:
    ...
    if df_funding.empty:
        st.info("目前抓不到資金費率資料，可能是交易所 API 暫時不可用，稍後再試。")
    else:
        only_negative = st.checkbox("只顯示負費率", value=False, key="funding_only_negative")
        search_kw = st.text_input("搜尋合約", key="funding_search", placeholder="例如 BTC")

        view = df_funding.copy()
        if only_negative:
            view = view[view["資金費率"] < 0]
        if search_kw:
            view = view[view["合約"].str.contains(search_kw.strip().upper(), case=False, na=False)]

        view = view.sort_values(by="資金費率", ascending=True)
...
def get_gate_futures_balance():
    ...
    exchange = ccxt.gate({
        ...
        'options': {'defaultType': 'swap'} # Gate.io 合約也使用 swap
    })
    
    # 查詢餘額
    balance = exchange.fetch_balance({'type': 'swap'})
    # Gate.io 的餘額結構通常在 'USDT' 欄位
    usdt_balance = balance['total'].get('USDT', 0)
    return usdt_balance
def get_futures_balance():
    # 這裡假設你已經在 secrets.toml 設定好金鑰
    # 如果你用的是 MEXC
    exchange = ccxt.mexc({
        'apiKey': st.secrets["MEXC_API_KEY"],
        'secret': st.secrets["MEXC_API_SECRET"],
        'options': {'defaultType': 'future'}  # 強制切換為合約模式
    })
    
    # 抓取餘額
    balance = exchange.fetch_balance()
    # 通常合約餘額在 'USDT' 欄位，或者根據交易所返回的格式調整
    usdt_balance = balance['total'].get('USDT', 0)
    return usdt_balance
